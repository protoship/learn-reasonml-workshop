...
let my_ints: list(int) = [1, 2, 3, 4, 5];

let double_my_ints = ints : list(int) => List.map(~f=x => x * 2, ints);

let () =
  assert (
    [%compare.equal: list(int)](double_my_ints(my_ints), [2, 4, 6, 8, 10])
  );

let my_strings = ints : list(string) => List.map(~f=Int.to_string, ints);

let () =
  assert (
    [%compare.equal: list(string)](
      my_strings(my_ints),
      ["1", "2", "3", "4", "5"],
    )
  );
...
/* Exercise: implement the value [my_new_ints], which is obtained by adding 1 to each
   element of [my_ints] */
let my_new_ints = ints => failwith("For you to implement");
...
/* If the function you want to perform on each element of your list is one that returns
      ...
      there is a higher-order function called [List.iter] which has the following signature:
        ...
        val iter
          ...
          -> unit
   */
let () = List.iter(~f=i => Stdio.printf("here's an int: %i\n", i), my_ints);
...
/* Here's one more example of a useful list function: [List.find]:
        ...
        val find
          ...
          -> 'a
   */
let first_num_greater_than_3 = ints => List.find_exn(~f=x => x > 3, ints);

let () = assert (first_num_greater_than_3(my_ints) == 4);

let%test "Testing my_new_ints..." =
  [%compare.equal: list(int)]([2, 3, 4, 5, 6], my_new_ints(my_ints));

let%test "Testing num_even_ints..." = Int.equal(2, num_even_ints(my_ints));
