...
let () =
  assert ([%compare.equal: option(int)](map_option(double, None), None));

let () =
  assert (
    [%compare.equal: option(int)](map_option(double, Some(2)), Some(4))
  );
...
let () =
  assert (
    [%compare.equal: option(int)](
      map_option(i => 2 * i, Some(2)),
      Some(4),
    )
  );
...
/* Define a function apply_if_nonzero which takes a function from
   ...
   */
/* TODO */
let apply_if_nonzero = (f, i) => failwith("For you to implement");

let%test "Testing apply_if_nonzero..." =
  Int.(==)(0, apply_if_nonzero(x => 10 / x, 0));

let%test "Testing apply_if_nonzero..." =
  Int.(==)(2, apply_if_nonzero(x => 10 / x, 5));
