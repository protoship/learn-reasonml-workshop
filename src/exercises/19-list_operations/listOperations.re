...
let double_my_ints = ints : list(int) => List.map(x => x * 2, ints);

/* let () =
   assert (
     [%compare.equal: list(int)](double_my_ints(my_ints), [2, 4, 6, 8, 10])
   ); */
let my_strings = ints : list(string) => List.map(string_of_int, ints);
...
/* If the function you want to perform on each element of your list is one that returns
      ...
      there is a higher-order function called [List.iter] which has the following signature:
        ...
        val iter
          ...
          -> unit
   */
/* let () = List.iter(i => Stdio.printf("here's an int: %i\n", i), my_ints); */
...
let sum_of_my_ints = ints : int =>
  List.fold_left((total, my_int) => total + my_int, 0, ints);
...
/* Here's one more example of a useful list function: [List.find]:
        ...
        val find
          ...
          -> 'a
   */
let first_num_greater_than_3 = ints => List.find(x => x > 3, ints);
